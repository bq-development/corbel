...
public class S3RestorDao implements RestorDao {
	...
	private RestorObject getObject(MediaType mediaType, String collection, String resource, int retryNumber) {
		GetObjectRequest objectRequest = new GetObjectRequest(bucket, keyNormalizer.normalize(mediaType, collection,
				resource));
		try {
			...
			return new RestorObject(objectType, s3Object.getObjectContent(), s3Object.getObjectMetadata()
					.getContentLength());
	...
	private void uploadObject(String collection, String resource, RestorObject object, int retryNumber) {
		...
		PutObjectRequest objectRequest = new PutObjectRequest(bucket, keyNormalizer.normalize(object.getMediaType(),
				collection, resource), object.getInputStream(), createObjectMetadataForObject(object.getMediaType(),
	...
	private void deleteObject(MediaType mediaType, String collection, String resource, int retryNumber) {
		try {
			amazonS3Client.deleteObject(new DeleteObjectRequest(bucket, keyNormalizer.normalize(mediaType, collection,
					resource)));
		} catch (AmazonS3Exception e) {
			switch (e.getStatusCode()) {
				case 500:
